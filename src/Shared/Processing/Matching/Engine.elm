module Shared.Processing.Matching.Engine exposing
    ( matchRows, exactMatch, fuzzyMatch, scoreSimilarity
    )
...
import Tools.DataExtractor.Model exposing (MatchedRecord, ProcessedData, ProcessingStats, MatchConfig)
...
matchRows config masterRows dataRows =
    let
        startTime = 0.0  -- In a real implementation, this would use current time
        
        matchedPairs = findMatches config masterRows dataRows
        matchedRecords = List.map (createMatchedRecord config) matchedPairs
        
        matchedMasterIndices = Set.fromList (List.map .masterIndex matchedPairs)
        matchedDataIndices = Set.fromList (List.map .dataIndex matchedPairs)
...
findMatches config masterRows dataRows =
    let
        masterIndexed = List.indexedMap Tuple.pair masterRows
        dataIndexed = List.indexedMap Tuple.pair dataRows
        
        allCandidates =
            masterIndexed
                |> List.concatMap (\(masterIndex, masterRow) ->
                    dataIndexed
                        |> List.filterMap (\(dataIndex, dataRow) ->
...
removeDuplicateMatches accepted remaining =
    case remaining of
        ...
        candidate :: rest ->
            let
                alreadyUsedMaster = List.any (.masterIndex >> (==) candidate.masterIndex) accepted
                alreadyUsedData = List.any (.dataIndex >> (==) candidate.dataIndex) accepted
            in
...
evaluateMatch config masterIndex dataIndex masterRow dataRow =
    let
        masterValues = getColumnValues config.masterColumns masterRow
        dataValues = getColumnValues config.dataColumns dataRow
        
        matchResults = List.map2 (evaluateFieldMatch config.useFuzzyMatch) masterValues dataValues
        matchingFields = List.filterMap identity matchResults
        
        totalFields = List.length config.masterColumns
        matchedFields = List.length matchingFields
        ...
        overallScore = 
            if totalFields == 0 then
                ...
                let
                    fieldScores = List.map .score matchingFields
                    totalScore = List.sum fieldScores
                in
...
evaluateFieldMatch useFuzzy masterValue dataValue =
    let
        cleanMaster = String.trim (String.toLower masterValue)
        cleanData = String.trim (String.toLower dataValue)
    in
...
createStats masterRows dataRows matchedRecords processingTime =
    let
        totalMaster = List.length masterRows
        totalData = List.length dataRows
        matched = List.length matchedRecords
    in
...
fuzzyMatch threshold str1 str2 =
    let
        validThreshold = max 0.0 (min 1.0 threshold)  -- Clamp threshold between 0 and 1
    in
...
scoreSimilarity str1 str2 =
    let
        clean1 = String.trim (String.toLower str1)
        clean2 = String.trim (String.toLower str2)
    in
    if clean1 == clean2 then
        ...
        let
            longer = max (String.length clean1) (String.length clean2)
            shorter = min (String.length clean1) (String.length clean2)
            ratio = toFloat shorter / toFloat longer
        in
