        ...
        , describe "Fuzzy Match Error Cases"
            [ test "handles invalid fuzzy match threshold" <|
                \_ ->
                    let
                        result1 =
                            Engine.fuzzyMatch -0.5 "test" "test"

                        -- Negative threshold
                        result2 =
                            Engine.fuzzyMatch 1.5 "test" "test"
            ...
            , test "handles empty strings in fuzzy matching" <|
                \_ ->
                    let
                        score1 =
                            Engine.scoreSimilarity "" "test"

                        score2 =
                            Engine.scoreSimilarity "test" ""

                        score3 =
                            Engine.scoreSimilarity "" ""
                    in
                    Expect.all
            ...
            , test "handles very long strings" <|
                \_ ->
                    let
                        ...
                        score =
                            Engine.scoreSimilarity longString "a"
                    in
                    Expect.all
            ...
            [ test "handles extremely large datasets" <|
                \_ ->
                    let
                        ...
                        masterRows =
                            List.range 1 largeNumber |> List.map (\i -> [ String.fromInt i ])
            ...
            , test "handles complex nested data structures" <|
                \_ ->
                    let
                        ...
                        processedData =
                            Engine.matchRows matchConfig complexMasterRows complexDataRows
                    in
            ...
            [ test "recovers from malformed match configuration" <|
                \_ ->
                    let
                        ...
                        processedData =
                            Engine.matchRows emptyConfig masterRows dataRows
                    in
            ...
            , test "handles mismatched column list lengths" <|
                \_ ->
                    let
                        ...
                        processedData =
                            Engine.matchRows mismatchedConfig masterRows dataRows
                    in
            ...
            [ test "preserves data integrity after errors" <|
                \_ ->
                    let
                        ...
                        totalRows =
                            processedData.statistics.matchedCount
            ...
            , test "handles data corruption gracefully" <|
                \_ ->
                    let
                        ...
                        processedData =
                            Engine.matchRows matchConfig corruptedMasterRows corruptedDataRows
                    in
