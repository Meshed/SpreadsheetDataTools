            ...
            [ test "limits preview to exactly 3 samples" <|
                \_ ->
                    let
                        ...
                        masterRows =
                            List.range 1 10 |> List.map (\i -> [ "Name" ++ String.fromInt i, String.fromInt i ])
                        ...
                        fullProcessedData =
                            Engine.matchRows matchConfig masterRows dataRows

                        limitedData =
                            Preview.limitToPreviewSize 3 fullProcessedData
                    in
                    Expect.all
            ...
            , test "handles fewer than 3 matches correctly" <|
                \_ ->
                    let
                        ...
                        limitedData =
                            Preview.limitToPreviewSize 3 processedData
                    in
            ...
            [ test "processes large dataset efficiently" <|
                \_ ->
                    let
                        ...
                        masterRows =
                            List.range 1 1000 |> List.map (\i -> [ "Name" ++ String.fromInt i, String.fromInt i ])
            ...
            , test "preview generation completes within 500ms for files under 10MB (AC 9)" <|
                \_ ->
                    let
                        ...
                        masterRows =
                            List.range 1 100 |> List.map (\i -> [ "Name" ++ String.fromInt i, String.fromInt i, "extra" ++ String.fromInt i ])

                        dataRows =
                            List.range 1 100 |> List.map (\i -> [ "Name" ++ String.fromInt i, String.fromInt i, "dept" ++ String.fromInt i ])
            ...
            , test "memory usage validation for preview processing (AC 10)" <|
                \_ ->
                    let
                        ...
                        masterRows =
                            List.range 1 500 |> List.map (\i -> List.repeat 10 ("data" ++ String.fromInt i))
                        ...
                        originalMatchCount =
                            List.length processedData.matchedRecords

                        limitedMatchCount =
                            List.length limitedData.matchedRecords
                    in
                    Expect.all
        ...
        , describe "Memory Efficiency"
            [ test "preserves original data structure integrity" <|
                \_ ->
                    let
                        ...
                        limitedData =
                            Preview.limitToPreviewSize 3 originalData

                        -- First record should be preserved exactly
                        firstOriginal =
                            List.head originalData.matchedRecords

                        firstLimited =
                            List.head limitedData.matchedRecords
                    in
            ...
            , test "does not modify statistics when limiting" <|
                \_ ->
                    let
                        masterRows =
                            List.range 1 100 |> List.map (\i -> [ "Name" ++ String.fromInt i, String.fromInt i ])

                        dataRows =
                            List.range 1 50 |> List.map (\i -> [ "Name" ++ String.fromInt i, String.fromInt i ])
            ...
            [ test "handles zero limit correctly" <|
                \_ ->
                    let
                        ...
                        limitedData =
                            Preview.limitToPreviewSize 0 processedData
                    in
                    Expect.equal 0 (List.length limitedData.matchedRecords)
            , test "handles negative limit correctly" <|
                \_ ->
                    let
                        ...
                        limitedData =
                            Preview.limitToPreviewSize -1 processedData
                    in
                    Expect.equal 0 (List.length limitedData.matchedRecords)
            , test "handles very large limit correctly" <|
                \_ ->
                    let
                        ...
                        limitedData =
                            Preview.limitToPreviewSize 1000 processedData
                    in
