            ...
            [ test "generates preview with valid data" <|
                \_ ->
                    let
                        ...
                        processedData =
                            Engine.matchRows matchConfig masterFile.rows dataFile.rows
                    in
        ...
        , describe "Match Configuration"
            [ test "exact match works correctly" <|
                \_ ->
                    let
                        result =
                            Engine.exactMatch "John Doe" "John Doe"
                    in
            ...
            , test "fuzzy match works with threshold" <|
                \_ ->
                    let
                        score =
                            Engine.scoreSimilarity "John" "John Doe"

                        result =
                            Engine.fuzzyMatch 0.6 "John" "John Doe"

                        -- Lower threshold since "John" in "John Doe" gives ~0.64
                    in
                    Expect.equal True result
        ...
        , describe "Error Handling"
            [ test "handles missing master file" <|
                \_ ->
                    let
                        model =
                            { init | masterFile = Nothing, dataFile = Just sampleDataFile, matchConfig = Just sampleMatchConfig }
            ...
            , test "handles missing data file" <|
                \_ ->
                    let
                        model =
                            { init | masterFile = Just sampleMasterFile, dataFile = Nothing, matchConfig = Just sampleMatchConfig }
                    in
            ...
            , test "handles missing match config" <|
                \_ ->
                    let
                        model =
                            { init | masterFile = Just sampleMasterFile, dataFile = Just sampleDataFile, matchConfig = Nothing }
                    in
