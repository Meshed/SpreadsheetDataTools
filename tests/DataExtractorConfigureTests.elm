    ...
    describe "Column Selection Tests"
        [ test "SelectMasterColumn adds column to selectedMasterColumns" <|
            \_ ->
                let
                    msg =
                        ConfigureMsg (SelectMasterColumn "Employee_ID")
        ...
        , test "SelectMasterColumn does not add duplicate column" <|
            \_ ->
                let
                    model =
                        { initialModel | selectedMasterColumns = [ "Employee_ID" ] }
        ...
        , test "DeselectMasterColumn removes column from selectedMasterColumns" <|
            \_ ->
                let
                    model =
                        { initialModel | selectedMasterColumns = [ "Employee_ID", "Full_Name" ] }

                    msg =
                        ConfigureMsg (DeselectMasterColumn "Employee_ID")
        ...
        , test "SelectDataColumn adds column to selectedDataColumns" <|
            \_ ->
                let
                    msg =
                        ConfigureMsg (SelectDataColumn "EmpID")
        ...
        , test "DeselectDataColumn removes column from selectedDataColumns" <|
            \_ ->
                let
                    model =
                        { initialModel | selectedDataColumns = [ "EmpID", "Name" ] }

                    msg =
                        ConfigureMsg (DeselectDataColumn "EmpID")
    ...
    describe "Selection Ordering Tests"
        [ test "Multiple selections maintain order of addition" <|
            \_ ->
                let
                    ...
                    msg2 =
                        ConfigureMsg (SelectMasterColumn "Full_Name")

                    ( model2, _ ) =
                        update msg2 model1

                    msg3 =
                        ConfigureMsg (SelectMasterColumn "Email")
        ...
        , test "Removing middle selection preserves order of remaining items" <|
            \_ ->
                let
                    model =
                        { initialModel | selectedMasterColumns = [ "Employee_ID", "Full_Name", "Email" ] }

                    msg =
                        ConfigureMsg (DeselectMasterColumn "Full_Name")
    ...
    describe "MatchConfig Generation Tests"
        [ test "ToggleFuzzyMatching creates MatchConfig when none exists" <|
            \_ ->
                let
                    msg =
                        ConfigureMsg (ToggleFuzzyMatching True)
        ...
        , test "ToggleFuzzyMatching updates existing MatchConfig" <|
            \_ ->
                let
                    existingConfig =
                        { masterColumns = [], dataColumns = [], useFuzzyMatch = False }

                    model =
                        { initialModel | matchConfig = Just existingConfig }
